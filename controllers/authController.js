...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (currentUser.changedPasswordAfter(decoded.iat)) {
    return next(
      new AppError(
        'Vous changez votre mot de passe recemment. Veuillez vous reconnecter.',
        401
      )
    );
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError("Il n'y pas d'utilisateur avec cet email.", 404));
  }
  ...
  try {
    ...
    return next(
      new AppError(
        "Il y'a eu une erreur de l'envoi du mail. Reessayer plus tard!",
        500
      )
    );
  }
